...
export async function generateClientContractPdf(c: ContractDocument): Promise<GeneratedPdfResult> {
  ...
  const drawHeader = (p: typeof page) => {
    ...
    p.drawText('TECNOJACK · PRODUCCIÓN AUDIOVISUAL', {
      x: margin,
    ...
    p.drawText(`DOCUMENTO OFICIAL · CONTRATO N° ${c.contractNumber}`, {
  ...
  page.drawText('CONTRATO DE PRESTACIÓN DE SERVICIOS AUDIOVISUALES Y FIRMA ELECTRÓNICA', {
    x: margin,
    y: y - 20,
    size: 11,
    font: fontBold,
  ...
  page.drawText('1. DATOS DEL CLIENTE Y DETALLES DEL EVENTO', {
    x: margin + 12,
    y: y - 18,
    size: 9,
    font: fontBold,
  ...
  page.drawText(`Cliente Contratante: ${c.client.fullName || 'N/A'}`, {
    x: margin + 12,
    y: y - 34,
    size: 8.5,
    font: fontBold,
  ...
  page.drawText(`Identificación: ${c.client.documentType || 'CC'} ${c.client.documentNumber || 'N/A'}`, {
    x: margin + 280,
    y: y - 34,
    size: 8.5,
    font: fontRegular,
  ...
  page.drawText(`Correo Electrónico: ${c.client.email || 'N/A'}`, {
    x: margin + 12,
    y: y - 48,
    size: 8.5,
    font: fontRegular,
  ...
  page.drawText(`Teléfono / WhatsApp: ${c.client.phone || 'N/A'}`, {
    x: margin + 280,
    y: y - 48,
    size: 8.5,
    font: fontRegular,
  ...
  page.drawText(`Ciudad y Dirección: ${c.client.city || 'Medellín'} ${c.client.address ? '· ' + c.client.address : ''}`, {
    x: margin + 12,
    y: y - 62,
    size: 8.5,
    font: fontRegular,
  ...
  page.drawText(`Fecha del Evento: ${c.service.eventDate || 'Por definir'}`, {
    x: margin + 280,
    y: y - 62,
    size: 8.5,
    font: fontRegular,
  ...
  page.drawText(`Locación del Evento: ${c.service.location || 'Por definir'}`, {
    x: margin + 12,
    y: y - 76,
    size: 8.5,
    font: fontRegular,
  ...
  page.drawText('2. RESUMEN ECONÓMICO Y ANTICIPO CONFIRMADO', {
    x: margin + 12,
    y: y - 18,
    size: 9,
    font: fontBold,
  ...
  page.drawText(`Paquete Seleccionado: ${c.service.packageName}`, {
    x: margin + 12,
    y: y - 34,
    size: 8.5,
    font: fontBold,
  ...
  page.drawText(`Valor Total del Contrato: ${formatCurrency(c.payment.totalAmount, c.payment.currency)}`, {
    x: margin + 12,
    y: y - 48,
    size: 8.5,
    font: fontBold,
  ...
  page.drawText(`Anticipo Confirmado (40% Recibido): ${formatCurrency(c.payment.paidAmount, c.payment.currency)} (${c.payment.paidPercentage}%)`, {
    x: margin + 260,
    y: y - 48,
    size: 8.5,
    font: fontBold,
  ...
  page.drawText(`Saldo Pendiente a Cancelar: ${formatCurrency(c.payment.remainingAmount, c.payment.currency)}`, {
    x: margin + 12,
    y: y - 62,
    size: 8.5,
    font: fontBold,
  ...
  page.drawText('3. ACEPTACIONES LEGALES Y POLÍTICAS REGISTRO DIGITAL', {
    x: margin + 12,
    y: y - 16,
    size: 9,
    font: fontBold,
  ...
  page.drawText(`[✓] Términos y Condiciones Generales (https://tecnojack.co/terminos-y-condiciones)`, {
    x: margin + 12,
    y: y - 30,
    size: 8,
    font: fontRegular,
  ...
  page.drawText(`[✓] Política de Tratamiento de Datos Personales (Habeas Data)`, {
    x: margin + 12,
    y: y - 42,
    size: 8,
    font: fontRegular,
  ...
  page.drawText(`[✓] Confirmación de Exactitud de la Información Suministrada`, {
    x: margin + 12,
    y: y - 54,
    size: 8,
    font: fontRegular,
  ...
  page.drawText(`[✓] Consentimiento de Firma Electrónica e Inmutabilidad`, {
    x: margin + 12,
    y: y - 66,
    size: 8,
    font: fontRegular,
  ...
  const imgChoiceText = acc.imageUseChoice === 'authorized'
    ...
    : 'No autorizado para uso promocional público';

  page.drawText(`Uso de Imagen: ${imgChoiceText}`, {
    x: margin + 280,
    y: y - 66,
    size: 8,
    font: fontBold,
  ...
  page.drawText('4. CLAUSULADO LEGAL OFICIAL DEL CONTRATO', {
    x: margin,
    y,
    size: 10,
    font: fontBold,
  ...
  for (const para of paragraphs) {
    ...
    const isHeading = /^\d+\.\s+[A-Z]/.test(cleanPara) || /^CONTRATO N°:/.test(cleanPara) || /^CLÁUSULA/.test(cleanPara);
  ...
  page.drawText('5. FIRMA ELECTRÓNICA E INMUTABILIDAD DEL REGISTRO', {
    x: margin + 12,
    y: y - 16,
    size: 9,
    font: fontBold,
  ...
  if (sig?.signatureDataUrl && sig.signatureDataUrl.startsWith('data:image/png;base64,')) {
    try {
      ...
      page.drawText(`[FIRMA ELECTRÓNICA REGISTRADA EN SISTEMA]`, {
        x: margin + 12,
        y: y - 60,
        size: 9,
        font: fontBold,
    ...
    page.drawText(`FIRMA DIGITAL: ${signerName}`, {
      x: margin + 12,
      y: y - 60,
      size: 11,
      font: fontBold,
  ...
  page.drawText(`FIRMADO POR: ${signerName}`, {
    x: margin + 220,
    y: y - 40,
    size: 8.5,
    font: fontBold,
  ...
  page.drawText(`DOCUMENTO: ${signerDoc}`, {
    x: margin + 220,
    y: y - 54,
    size: 8.5,
    font: fontRegular,
  ...
  page.drawText(`FECHA Y HORA: ${signedAt}`, {
    x: margin + 220,
    y: y - 68,
    size: 8.5,
    font: fontRegular,
  ...
  page.drawText(`MÉTODO DE FIRMA: ${sig?.method || 'Firma electrónica trazable'}`, {
    x: margin + 220,
    y: y - 82,
    size: 8.5,
    font: fontRegular,
  ...
  page.drawText(`COMPROBANTE DE INTEGRIDAD SHA-256: ${sha256Hex}`, {
    x: margin + 12,
    y: y - 110,
    size: 7.5,
    font: fontOblique,
