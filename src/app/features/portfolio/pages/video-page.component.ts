...
import { VideoAccordionCategory, VideoAccordionComponent } from './video-accordion.component';
import { VideoModalComponent, VideoServicePackage } from './video-modal.component';

const videoAdditionalServices: VideoServicePackage['additionalServices'] = [
  {
    name: 'Tomas con drone',
    price: 120000,
    description: 'Planos aéreos para ampliar el impacto visual y la escala del proyecto'
  },
  {
    name: 'Reel adicional para redes',
    price: 70000,
    description: 'Versión extra en formato vertical pensada para Instagram, TikTok y estados'
  },
  {
    name: 'Miniatura para YouTube / Spotify',
    price: 50000,
    description: 'Miniatura optimizada para YouTube y portada visual para Spotify'
  },
  {
    name: 'Creación de guión',
    price: 50000,
    description: 'Desarrollo estructurado de la narrativa del video con enfoque profesional'
  },
  {
    name: 'Conceptualización y storytelling',
    price: 120000,
    description: 'Construcción de idea creativa, estética y narrativa del video'
  },
  {
    name: 'Video de referencia previo',
    price: 80000,
    description: 'Video guía con referencias visuales para definir el resultado final antes de grabar'
  },
  {
    name: 'Dirección creativa extendida',
    price: 100000,
    description: 'Acompañamiento completo en la dirección artística durante todo el rodaje'
  },
  {
    name: 'Planificación de escenas',
    price: 70000,
    description: 'Organización de tomas y estructura visual para optimizar la grabación'
  },
  {
    name: 'Scouting de locación',
    price: 60000,
    description: 'Búsqueda y selección de locaciones adecuadas para el proyecto'
  }
];
...
@Component({
  ...
  standalone: true,
  imports: [NgFor, NgIf, PortfolioShellComponent, VideoAccordionComponent, VideoModalComponent, FallbackImageDirective, RevealOnScrollDirective],
...
export class VideoPageComponent implements OnInit {
  ...
  readonly heroFacts = computed(() => {
    ...
    const totalVideos = this.orderedCategories().reduce((sum, cat) => sum + cat.videos.length, 0);
    ...
    return [
      ...
      { label: 'Categorías', value: String(this.orderedCategories().filter((c) => c.videos.length).length) }
    ];
  });
  ...
  readonly heroSocialLinks = computed(() => {
    ...
    return links
      .map((link) => ({
        ...
        title: link.title,
        href: link.platform === 'whatsapp' ? this.content.buildWhatsappHref('Hola TECNOJACK, me interesa conocer más sobre los paquetes de video.') : link.href,
  ...
  readonly videoPackages: VideoServicePackage[] = [
    {
      ...
      deliverables: ['Video musical hasta 4 minutos', 'Resolución 4K', 'Edición básica', 'Entrega digital'],
    ...
    },
    {
      ...
      isBasePrice: true,
      categoryTag: 'Musical',
      description: 'Producción narrativa con enfoque cinematográfico para proyectos de alto nivel visual.',
      features: [
      ...
      deliverables: ['Video hasta 7 minutos', 'Edición cinematográfica', 'Color grading avanzado', 'Concepto visual definido'],
      additionalServices: videoAdditionalServices
    },
    {
      ...
      isBasePrice: true,
      categoryTag: 'Musical',
      description: 'Producción completamente adaptada a las necesidades del cliente.',
      features: [
    ...
    },
    {
      ...
      description: 'Producción narrativa para cortometrajes, storytelling o proyectos creativos no musicales.',
      features: [
      ...
      deliverables: ['Cortometraje o pieza narrativa', 'Duración variable', 'Edición cinematográfica', 'Adaptación según proyecto'],
      additionalServices: [
        {
          name: 'Guión o estructura narrativa',
          price: 50000,
          description: 'Desarrollo del guión o estructura narrativa del cortometraje'
        },
        {
        ...
        },
        {
          ...
          price: 70000,
          description: 'Desarrollo de arco narrativo y estructura emocional del relato'
        },
        {
          name: 'Desarrollo de idea narrativa',
          price: 60000,
          description: 'Conceptualización de la historia a contar antes del rodaje'
        },
        {
          name: 'Dirección creativa durante rodaje',
          price: 100000,
          description: 'Acompañamiento artístico completo durante todo el día de grabación'
        },
        {
          name: 'Planificación de escenas',
          price: 70000,
        ...
        },
        {
          name: 'Tomas con drone',
          price: 120000,
          description: 'Planos aéreos para ampliar la escala visual del cortometraje'
        }
      ]
    }
  ];
  ...
  private readonly orderedCategories = computed<VideoAccordionCategory[]>(() => {
    ...
    const musicales = categories.find((category) => category.key === 'musicales');
    ...
    const redesPattern = /\b(reel|reels|social|vertical|teaser|redes|lightroom|edici(o|ó)n|edit(ing)?|color grading|gradi[gn]g)\b/i;
    const redesVideos = otrosVideos.filter((video) => redesPattern.test(video.title));
    const eventosVideos = otrosVideos.filter((video) => !redesPattern.test(video.title));
  ...
  readonly featuredVideos = computed(() => this.orderedCategories()[0]?.videos.slice(0, 6) ?? []);

  constructor() {
    effect((onCleanup) => {
      const modalOpen = !!this.selectedVideo() || !!this.selectedVideoPackage();
      this.document.body.classList.toggle('portfolio-request-modal-open', modalOpen);
      this.document.documentElement.classList.toggle('portfolio-request-modal-open', modalOpen);

      onCleanup(() => {
        this.document.body.classList.remove('portfolio-request-modal-open');
        this.document.documentElement.classList.remove('portfolio-request-modal-open');
      });
    });
  }
  ...
  videoPackageImage(pkg: VideoServicePackage): string {
    switch (pkg.name) {
      case 'Video Esencial':
  ...
  get headerCtaHref() {
    return this.content.buildWhatsappHref('Hola TECNOJACK, me interesa conocer más sobre los paquetes de video.');
  }
}
