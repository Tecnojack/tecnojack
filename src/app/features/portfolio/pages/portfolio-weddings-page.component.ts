...
@Component({
  selector: 'tj-portfolio-weddings-page',
  standalone: true,
...
export class PortfolioWeddingsPageComponent {
  private readonly content = inject(PortfolioContentService);
  private readonly title = inject(Title);
  private readonly meta = inject(Meta);

  readonly category: PortfolioPackageCategory = 'bodas';
  ...
  constructor() {
    effect(() => {
      const pageMeta = this.content.getPageMeta(
        'portfolio-bodas',
        'TECNOJACK | Bodas',
        'Paquetes de boda, foto y video con dirección visual cinematográfica.'
      );
