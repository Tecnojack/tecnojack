...
@Component({
  selector: 'tj-portfolio-quinces-page',
  standalone: true,
...
export class PortfolioQuincesPageComponent {
  private readonly content = inject(PortfolioContentService);
  private readonly title = inject(Title);
  private readonly meta = inject(Meta);

  readonly category: PortfolioPackageCategory = 'quinces';
  ...
  constructor() {
    effect(() => {
      const pageMeta = this.content.getPageMeta(
        'portfolio-quinces',
        'TECNOJACK | Quinces',
        'Paquetes visuales para quinceañeras con foto, video y entregas premium.'
      );
